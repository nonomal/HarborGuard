...
export class PatchExecutorTar {
  ...
  private async importTarToBuildah(tarPath: string): Promise<string> {
    ...
    // Import the tar as a new image in Buildah
    const imageName = `patch-working-${Date.now()}`;
    await execAsync(`buildah --storage-driver vfs from docker-archive:${tarPath}`);
    
    // Get the container ID
    const { stdout } = await execAsync(`buildah --storage-driver vfs containers --format "{{.ContainerID}}" | head -1`);
