...
export function VersionNotification() {
  ...
  // Don't show if no update available, loading, dismissed, or if version checking is disabled
  if (!hasUpdate || loading || dismissed) {
    return null;
  }

  // Don't show for certain errors (like version checking disabled)
  if (error && (error.includes("disabled") || error.includes("unavailable"))) {
    return null;
  }
  ...
  return (
    <>
      ...
      <Card className="mx-2 mb-2 border-blue-200 rounded-sm bg-white dark:border-blue-800 dark:bg-blue-950 !py-2 !px-0">
        <CardContent onClick={handleViewDetails} className="!px-2 cursor-pointer">
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm">
                  ...
                  <code className="block mt-2 p-2 bg-amber-100 dark:bg-amber-900/50 rounded text-xs font-mono">
                    docker pull ghcr.io/harborguard/harborguard:latest && docker
                    restart harborguard
                  </code>
                </div>
              </div>
